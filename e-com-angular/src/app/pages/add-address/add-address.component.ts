...
export class AddAddressComponent implements OnInit {
  ...
  fetchUserCart(): void {
    this.http
      .get(`${this.baseUrl}/api/cart/getCartItem/${this.userId}`)
      .subscribe({
        next: (response: any) => {
  ...
  fetchUserAddresses(): void {
    this.http
      .post(`${this.baseUrl}/api/user/get-user-addresses`, {
        userId: this.userId,
      })
      .subscribe({
        next: (response: any) => {
  ...
  handleSubmit(): void {
    this.http
      .post(`${this.baseUrl}/api/user/add-address`, {
        userId: this.userId,
  ...
  handleDeleteAddress(index: number): void {
    this.http
      .post(`${this.baseUrl}/api/user/delete-user-address`, {
        userId: this.userId,
        index,
      })
      .subscribe({
  ...
  makePayment(): void {
    loadStripe(
      'pk_test_51Q3XBV06SFttaIhQGO4iDvYeq1aZUD5iydeSE4oKTjNEWqhENI2LhxS5zP6WLrHehvC0ERtrZTc0gx4Yl1Y4nZ0600rwoOIqai'
    ).then((stripe) => {
      const token = localStorage.getItem('token');
      ...
      this.http
        .post(`${this.baseUrl}/api/payment/create-checkout-session`, body)
        .subscribe({
          next: (session: any) => {
  ...
  handleProceedToPay(): void {
    if (this.userDeliveryAddress) {
      localStorage.setItem(
        'userDeliveryAddress',
        JSON.stringify(this.userDeliveryAddress)
      );
      this.makePayment();
    } else {
